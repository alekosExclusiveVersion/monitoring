...
Режимы:
  ...
  --scope active   (по умолчанию) только неархивные проекты с активной услугой;
  --scope live     для ручной сверки: активная услуга или пользователь в
                   System Monitor (в уведомление такие проекты не идут);
  --scope hosted   для ручной сверки: все неархивные проекты сервера;
  --json           машинный вывод;
  --refresh        обновить кэш принудительно;
  --no-extended    только группы 1/14 (по умолчанию также 53/74/75);
